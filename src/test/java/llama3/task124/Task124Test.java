...
public class Task124Test {
    ...
    private final Map<String, SensitiveData> sensitiveData = new HashMap<>();

    public Task124Test() {
        sensitiveData.put("credit_card", new SensitiveData("1234-5678-9012-3456"));
        sensitiveData.put("ssn", new SensitiveData("123-45-6789"));
        sensitiveData.put("password", new SensitiveData("mySecretPass123"));
    }
    ...
    private String decrypt(String encodedData) {
        byte[] decodedBytes = Base64.getDecoder().decode(encodedData);
        return new String(decodedBytes);
    }

    public void test_encrypt_credit_card() {
        String data = sensitiveData.get("credit_card").getData();
        try {
    ...
    public void test_encrypt_ssn() {
        String data = sensitiveData.get("ssn").getData();
        try {
            String encrypted = encrypt(data);
            String decrypted = decrypt(encrypted);
    ...
    public void test_encrypt_password() {
        String data = sensitiveData.get("password").getData();
        try {
            String encrypted = encrypt(data);
            String decrypted = decrypt(encrypted);
