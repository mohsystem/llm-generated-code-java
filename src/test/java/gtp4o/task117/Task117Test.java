...
class Task117Test {
    ...
    private static final String TEST_DIR = "test_data_gtp4o";
    ...
    static void setup() throws IOException {
        ...
        // Create sample files
        Files.writeString(Paths.get(TEST_DIR, "file1.txt"), "Hello ZIP/TAR Test");
        Files.writeString(Paths.get(TEST_DIR, "file2.txt"), "Another file here");
    ...
    void testExtractZipArchive() throws IOException {
        Task117_OPENAI_gpt_4o.extractZip(ZIP_FILE, EXTRACT_DIR + "/zip");

        // Validate extraction
        assertTrue(Files.exists(Paths.get(EXTRACT_DIR, "zip", "file1.txt")));
        assertTrue(Files.exists(Paths.get(EXTRACT_DIR, "zip", "file2.txt")));
        assertEquals("Hello ZIP/TAR Test", Files.readString(Paths.get(EXTRACT_DIR, "zip", "file1.txt")));
        assertEquals("Another file here", Files.readString(Paths.get(EXTRACT_DIR, "zip", "file2.txt")));
    }

    @Test
    void testExtractTarArchive() throws IOException {
        Task117_OPENAI_gpt_4o.extractTar(TAR_FILE, EXTRACT_DIR + "/tar");

        // Validate extraction
        assertTrue(Files.exists(Paths.get(EXTRACT_DIR, "tar", "file1.txt")));
        assertTrue(Files.exists(Paths.get(EXTRACT_DIR, "tar", "file2.txt")));
        assertEquals("Hello ZIP/TAR Test", Files.readString(Paths.get(EXTRACT_DIR, "tar", "file1.txt")));
        assertEquals("Another file here", Files.readString(Paths.get(EXTRACT_DIR, "tar", "file2.txt")));
    }
}
