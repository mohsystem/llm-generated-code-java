...
public class Task135Test {
    ...
    static class MockRequest implements HttpServletRequest {
        ...
        private final HttpSession session = new MockSession();
    ...
    public void testValidToken() throws ServletException, IOException {
        Task135_MISTRAL_codestral_latest servlet = new Task135_MISTRAL_codestral_latest();

        MockRequest request = new MockRequest();
        MockResponse response = new MockResponse();

        String token = "secure_token_123";
        request.setParameter("csrf_token", token);
        request.getSession().setAttribute("csrf_token", token);

        servlet.doPost(request, response);

        // if valid, servlet should not call sendError
        assertEquals(200, response.getStatus(), "Expected status 200 on valid token");
    ...
    public void testInvalidToken() throws ServletException, IOException {
        ...
        request.setParameter("csrf_token", "bad_token");
        request.getSession().setAttribute("csrf_token", "real_token");

        servlet.doPost(request, response);

        assertEquals(400, response.getStatus(), "Expected 400 on invalid token");
        assertEquals("CSRF token validation failed", response.getBody());
    }
}
